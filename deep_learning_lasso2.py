...
import optuna
import joblib  # Для сохранения и загрузки scaler'ов

# === Настройки ===
target_column = 'mid_price'
csv_filename = "predictions_lstm_torch.csv"
feature_sets = ['lasso']  # Оставлен только Lasso
...
def train_evaluate_lstm(X_train, y_train, X_val, y_val, X_test, y_test, prefix, optuna_trial=None):
    ...
    joblib.dump(y_scaler, f'y_scaler_{prefix}.pkl')
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch + 1}/{epochs}, Train Loss: {train_loss / len(train_loader):.4f}, Val Loss: {avg_val_loss:.4f}")
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            no_improvement = 0
            torch.save(model.state_dict(), f"best_model_{prefix}.pth")  # Сохранение модели
            torch.save(model, f"full_model_{prefix}.pt")  # Сохранение полной модели
        else:
            no_improvement += 1
    ...
    print(
        f"LSTM на {prefix}: R²={test_metrics['R²']:.4f}, MAE={test_metrics['MAE']:.4f}, RMSE={test_metrics['RMSE']:.4f}")
    results.append({
        'Model': 'LSTM',
...
def load_model_and_predict(input_data, prefix):
    model = torch.load(f"full_model_{prefix}.pt")
    model.eval()
    scaler = joblib.load(f'scaler_{prefix}.pkl')
    y_scaler = joblib.load(f'y_scaler_{prefix}.pkl')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)

    # Преобразуем вход в нужную форму
    input_scaled = scaler.transform(input_data)
    input_seq = torch.tensor(input_scaled[-lookback:], dtype=torch.float32).unsqueeze(0).to(device)

    with torch.no_grad():
        pred = model(input_seq)
    pred_rescaled = y_scaler.inverse_transform(pred.cpu().numpy().reshape(-1, 1)).flatten()[0]
...
for prefix in feature_sets:
    ...
    final_results, final_y_true, final_y_pred = train_evaluate_lstm(
        X_train, y_train,
        X_val, y_val,
        X_test, y_test,
        prefix,
    ...
    best_preds_df.to_csv(f"best_lstm_preds_{prefix}.csv", index=False)
