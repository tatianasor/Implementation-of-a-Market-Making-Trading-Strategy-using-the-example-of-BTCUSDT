...
import pandas as pd
import numpy as np
from sklearn.preprocessing import StandardScaler  # Используем StandardScaler
...
class LSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, output_size, dropout=0.5):  # Увеличен dropout
        super(LSTMModel, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True, dropout=dropout)
        self.fc = nn.Linear(hidden_size, output_size)  # Убран дополнительный полносвязный слой
...
# === Обучение и оценка ===
def train_evaluate_lstm(X_train, y_train, X_val, y_val, X_test, y_test, prefix):
    results = []
    ...
    scaler = StandardScaler()  # Используем StandardScaler
    ...
    model.apply(init_weights)  # Инициализация весов
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            no_improvement = 0
            torch.save(model.state_dict(), f"best_model_{prefix}.pth")  # Сохраняем лучшую модель
        else:
            no_improvement += 1
...
for prefix in feature_sets:
    ...
    results = train_evaluate_lstm(X_train, y_train, X_val, y_val, X_test, y_test, prefix)
    all_results[prefix.upper()] = results

# === Сохраняем метрики ===
with pd.ExcelWriter('lstm_optimization_results_torch2.xlsx') as writer:
    for name, result in all_results.items():
        df = pd.DataFrame(result)
        df.set_index('Model', inplace=True)
        sheet_name = f'{name[:28]}_Results'  # Обрезаем до 28 символов
